...
Phase 3: レビュー・修正
  ...
  → 全量ラウンドは 2 回まで。R2 の修正は再レビューせず Phase 4 のスコープラウンドで見る。テストと記述に閉じる修正は次のラウンドを起こさない
...
`references/review-guide.md` を読み、その手順に従ってレビュー・修正を行う。全量レビューは R1 と R2 の 2 ラウンド。R1 で fix がゼロなら R2 は行わない。R2 の fix は修正・コミットして再レビューせず Phase 4 に進み、修正増分は Phase 4 後のスコープラウンドが見る。R3 以降の全量ラウンドは行わない。小規模は R2 も行わず、R1 の fix を修正・コミットして Phase 4 へ進む。テストファイルに閉じる修正（fix-test）と記述に閉じる修正（fix-editorial）はそのラウンドで適用し、次のラウンドを起こさない。
...
- **変更起因の FAIL** — 修正方針フェーズ（`../_shared/references/review-loop.md` の同名節。**審査区分** 1 体）に通してから修正する。方針体には FAIL した TC の results と観測ログ、testing.md の該当項目、`plan.md`、変更ファイル一覧を渡し、FAIL ごとの修正設計（守る挙動・及ぶ経路・形）を返させる。方向が割れるものは要確認としてメインが決めるかユーザーに委ねる。修正設計を転記してサブエージェント（**判断区分**）に委譲して修正し（委譲方式とモデル選択は `../_shared/references/subagent-policy.md` に従う）、受け取り条件（変異の証跡・同根の経路・記述の根拠）を満たしてからコミット・push して、FAIL したケースと修正の影響範囲に絞って再検証する。修正のレビューは検証通過後のスコープラウンドがまとめて行う
...
- メインはオーケストレーションに徹し、実作業（実装・修正）はサブエージェントに委譲する。実装・修正は判断区分、R1 のレビューと方針立案（Phase 4 の変更起因の修正を含む）は審査区分、R2 以降のレビューは判断区分、関連コードの所在調査やコマンドの裏取りといった下調べは探索区分。委譲方式・並列実行・モデル選択・返り値の上限は `../_shared/references/subagent-policy.md` に従う
