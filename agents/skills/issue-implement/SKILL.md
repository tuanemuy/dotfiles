...
- **手順書起因の FAIL** — 実装は spec に合っているが、`testing.md` 側の手順・期待結果が誤っている場合。手順書を修正して該当項目だけ再実行する。コードに触れないため**検証サイクルは消費しない**。正しい実装を誤った手順書に合わせて「修正」しない — 迷ったら spec 本体が正。修正した手順書は、このフェーズを抜ける前に**手順書スコープラウンド**でレビューを1回通し（`references/review-guide.md` の「Phase 4 からの戻り」）、コミット・push して PR に反映する
...
- メインはオーケストレーションに徹し、実作業（実装・修正）はサブエージェントに委譲する。実装・修正は判断区分、レビュー・方針立案は審査区分、関連コードの所在調査やコマンドの裏取りといった下調べは探索区分。委譲方式・並列実行・モデル選択・返り値の上限は `../_shared/references/subagent-policy.md` に従う
