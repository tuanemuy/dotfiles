...
| `adr.md` | 設計判断 | Phase 3 の仕分け・Phase 7 の昇格判定（追記時は連番だけ引く） |
...
`.thread/{Issue番号}/adr.md` が存在すれば、各エントリを `../_shared/references/adr-guide.md` の記録基準（寿命テスト・波及テスト）にかけ、満たすものだけをプロジェクトルートの `.adr/{Issue番号}-{slug}.md` に転記する（ディレクトリがなければ新設する）。**`.adr/` の既存ファイルを数えて連番を採らない** — 並行ブランチと衝突するため、番号はこの Issue 番号を使う。1つの Issue から複数昇格する場合は slug で分ける。転記の際は adr.md 内の連番見出しを落とす（`## ADR-003: X` → `# ADR: X`）— 識別子はファイル名。転記したエントリには adr.md 側に「→ `.adr/{ファイル名}` に昇格」と一行追記する。基準を満たさないエントリは作業履歴としてそのまま残す — adr.md 自体は削除しない。
...
- メインはオーケストレーションに徹し、実作業（実装・修正）はサブエージェントに委譲する。実装・レビュー・修正は判断区分、関連コードの所在調査やコマンドの裏取りといった下調べは探索区分。委譲方式・並列実行・モデル選択は `../_shared/references/subagent-policy.md` に従う
