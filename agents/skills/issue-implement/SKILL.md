...
```text
Phase 0: フロー判定
  Issue を読みフル / ライトを判定（ユーザーの明示指定が最優先）
...
`gh issue view {番号}` で Issue を読み、フル / ライトを判定してから始める。**ユーザーの明示指定が最優先。** 依頼の意図で読む — 工程を省いた素早い対応を求めていればライト、計画・検証を含む丁寧な進行を求めていればフル。どちらとも読めない依頼は明示指定なしとして扱い、下の基準で判定する。
...
1つでも欠ける・迷ったらフル。判定と理由を一言宣言し、ライトなら `references/light-path-guide.md` に従って完結する（以降のフェーズは読まない）。フルなら Phase 1 へ。
...
`references/review-guide.md` を読み、その手順に従ってレビュー・修正を行う。全量レビューは R1 と R2 の 2 ラウンド。R1 で fix がゼロなら R2 は行わない。R2 の fix は修正・コミットして再レビューせず Phase 4 に進み、修正増分は Phase 4 後のスコープラウンドが見る。R3 以降の全量ラウンドは行わない。
...
観測は `../manual-test/SKILL.md` の手順に従って manual-test が担い、検証環境の起動・シードデータの準備も manual-test に委ねる。期待結果との突き合わせと合否判定はメインが行う。
