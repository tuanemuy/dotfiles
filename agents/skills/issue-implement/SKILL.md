...
Phase 6: ダッシュボード更新
  → ダッシュボード Issue があれば issue-dashboard で最新化（この Issue の進行・新規起票を反映）
```
...
- 存在する場合は `../issue-dashboard/SKILL.md` を読み、その手順に従ってダッシュボード Issue を更新する。
- この Issue は Phase 2 で PR を作成済みなので、issue-dashboard がオープン PR を読んで自動的に `(In Progress)` を付ける。実装側で特別な指定は不要。更新後、この Issue が作業中として表示されていることを確認する。
