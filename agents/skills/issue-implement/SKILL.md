...
- Phase 1: issue-planner に「小規模」を渡す。プランナーは方針レビューを行わない
...
観測ログが揃ったら、判定担当 1 体に合否と FAIL の三分を委譲する。メインは観測ログを読まない。
...
- **変更起因の FAIL** — 修正方針フェーズ（`../_shared/references/review-loop.md` の同名節。1 体）に通してから修正する。方針体には FAIL した TC の results と観測ログ、testing.md の該当項目、`plan.md`、変更ファイル一覧を渡し、FAIL ごとの修正設計（守る挙動・及ぶ経路・形）を返させる。方向が割れるものは要確認としてメインが決めるかユーザーに委ねる。修正設計を転記してサブエージェントに委譲して修正し（委譲方式は `../_shared/references/subagent-policy.md` に従う）、受け取り条件（変異の証跡・同根の経路・記述の根拠）を満たしてからコミット・push して、FAIL したケースと修正の影響範囲に絞って再検証する。修正のレビューは検証通過後のスコープラウンドがまとめて行う
...
スコープ外 Issue の起票・ダッシュボード更新・判断の昇格確認・レビューファイルの削除を、片付け担当 1 体に委譲する。メインは結果のサマリーだけ受け取り、完了報告に写す。
...
このブランチで触れたコードのコメントを整理する。**メインは `comment-cleanup/SKILL.md` を読まない** — 対象ファイル全文の読み込みと台帳作成でコンテキストが膨らむため、サブエージェントに委譲する（`../_shared/references/subagent-policy.md`）。
...
- メインはオーケストレーションに徹し、実作業（実装・修正）はサブエージェントに委譲する。委譲方式・並列実行・返り値の上限は `../_shared/references/subagent-policy.md` に従う
