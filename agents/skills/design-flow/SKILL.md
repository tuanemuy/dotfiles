---
name: design-flow
description: 設計ドキュメント（spec/）をベースに、UIデザインを段階的に作成するスキル。代表画面を「UI構造 × デザイン様式」の複数パターンでドラフト提案し、パターン選択を経てデザイントークンを定義し、全画面のHTMLデザインを作成・レビューする。ユーザーが「デザインして」「UIデザイン作って」「モック作って」「トークン定義して」「design the UI」などと言ったとき、または spec/ 成果物がある状態でデザイン作業を求められたら（間接的な依頼でも）トリガーする。途中再開も可能。
---
