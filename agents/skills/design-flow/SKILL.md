---
name: design-flow
description: "設計ドキュメント（spec/）をベースに、UIデザインを段階的に作成するスキル。代表画面を「UI構造 × デザイン様式」の複数パターンでドラフト提案し、パターン選択を経てデザイントークンを定義し、全画面のHTMLデザインを作成・レビューする。途中再開も可能。起動条件: `/design-flow` の実行、スキル名の明示的な指定、他スキルからの委譲のみ。「デザインして」「モック作って」のような一般的なデザイン依頼や、spec/ があるという状況だけでは自動起動しない。Figma のデザインの取り込みは figma-import を使う。"
user-invokable: true
---
...
デザイン担当エージェントには、この原則を作成条件として全文伝える。
...
メインエージェント（オーケストレーション）
  ├── デザイン担当エージェント（HTMLデザイン作成・修正）
  ├── agent-browser（スクリーンショット取得）
  └── デザインレビュアー（総合レビュー、Phase 4）
```

レビューは外部スキルに委譲せず、1体のデザインレビュアーが全画面をひとつのプロダクトとして評価する。

委譲方式は `../_shared/references/subagent-policy.md` に従う。

メインはスクリーンショットを抱え込まない。レビュアーが対象を自分で読み、レビュー全文をファイルに書き、メインには判定と指摘の一行リストだけを返す。
...
Phase 1: ドラフト提案
  代表画面を自動選定 → 「UI構造 × デザイン様式」の異なる3〜5案をHTML化
...
Phase 3: デザイン作成
  トークン・方針に基づき全画面のHTMLデザインを作成
  → agent-browser でスクリーンショットを取得し視覚的に確認
  → spec/design/pages/*.html を生成

Phase 4: レビュー
  スクリーンショット取得 → 全画面の総合デザインレビュー
  → 実質的な問題を修正し、問題がなくなるまで確認（最大3ラウンド）
```
...
このフェーズではデザイン担当エージェントがHTMLドラフトを作成する。
...
このフェーズではデザイン担当エージェントがHTMLデザインを作成する。
...
1体のレビュアーが全画面を総合的に評価し、実質的な問題だけを修正する。固定した細目でデザインを採点せず、プロダクトの目的と選択済みのデザイン意図に照らして専門的に判断する。
...
- 完成度またはユーザー体験に実質的な影響がある問題だけを記載する
- デザイン上の好み、成立する別案、根拠のない微調整は指摘しない
- 修正後に再レビューし、実質的な問題が0件なら完了する。最大3ラウンドで収束しない場合は、残った問題をユーザーに報告する
