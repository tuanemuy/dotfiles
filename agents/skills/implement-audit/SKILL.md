...
実装スキル（spec-implement・issue-implement）は省略を残さないことを原則とするが、大量のコード生成の過程で省略が紛れ込むことがある。このスキルは三段構え（パターンマッチング＋コード意味分析＋spec 照合）でスキャンし、構造化レポートを出力する。

**このスキルは検出・レポートのみ。修正は行わない。** 検出された問題の修正は、spec-implement の再開か、Issue を起票して issue-implement で行う。
...
対象スコープに対して、`references/detection-patterns.md` に定義されたパターンで検索する。機械的な検索なのでサブエージェントに委譲する。
...
照合対象ごとにサブエージェントを並列起動する。`{やること}` を差し替えて渡す:
...
3. 設計で定義されたメソッドやプロパティが実装されているかも確認する
...
1. spec/domains/ のポート定義（メソッドシグネチャ＋振る舞いの契約）を読む
2. 各ポートメソッドに対応するアダプター実装が存在するか、バックエンドごとに確認する
3. spec/testcases/ports/ の適合テストが実装され、各バックエンドに対して実行されているか確認する
4. 設計に存在するが実装が見つからない要素をリストアップする
...
検出結果に基づき、以下のレイヤーの修正を推奨する:

| 優先度 | レイヤー | 理由 | 推奨スキル |
|--------|---------|------|-----------|
| 1 | {layer} | Critical {数}件 | spec-implement の再開 or issue-implement |
```
...
- **検出ゼロ**（Critical / Warning / Info すべて0件）— `.audit/completeness/` を削除し、報告にその旨を記す。クリーンな結果の証明にレポートは要らない
- **検出あり** — レポートを残す。修正（spec-implement の再開・issue-implement）の入力になるため。修正が決着したらレポートは削除してよい
...
## 推奨アクション
{Critical・Warning が検出されたレイヤーについて、修正の推奨}

例:
- domain レイヤー: Critical 2件 → `spec-implement` を再開して未実装の要素を埋める
- frontend レイヤー: Warning 3件 → Issue を起票して `issue-implement` で対応
```
...
- **検出・レポートに専念する** — 修正は行わない。修正は spec-implement・issue-implement の役割
- **レイヤーを明示する** — 各検出項目にレイヤーを付与し、修正の範囲を判断できるようにする
