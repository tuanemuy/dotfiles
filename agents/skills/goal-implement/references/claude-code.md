...
Manager は Claude Code で開始するたびに、利用するプロジェクトに Stop hook があるかを確認する。`.claude/settings.json` の `hooks.Stop` に `goal-implement-stop.sh` を呼ぶ項目がなければ、次の手順で入れてユーザーに報告する。hook の設定はこのスキルの配布物には含めず、プロジェクト側に置く。

1. このファイルと同じディレクトリの `goal-implement-stop.sh` をプロジェクトの `.claude/hooks/goal-implement-stop.sh` にコピーする。
...
```json
{
  "hooks": {
    "Stop": [
      {
        "hooks": [
          { "type": "command", "command": "bash .claude/hooks/goal-implement-stop.sh" }
        ]
      }
    ]
  }
...
スクリプトは `status.json` の `terminal` が `null` の間、セッション終了をブロックして Manager を継続させる。継続回数はセッションごとに数え、`GOAL_IMPLEMENT_MAX_CONTINUES` (既定 50) で必ず抜ける。`status.json` がなければ何もしないため、他のセッションには影響しない。`{run-directory}` が `.goal-implement/{run-id}/` の場合は、スクリプト内の `status` と `counter` のパスを合わせる。Stop hook はメインエージェントの終了だけに発火し、サブエージェントには影響しない。
...
```bash
until [ "$(jq -r '.terminal' .goal-implement/status.json 2>/dev/null)" != "null" ]; do
  claude -p '$goal-implement を再開する' --max-turns 200
done
```
