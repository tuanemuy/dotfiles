...
メインエージェントはオーケストレーションに集中する。ブラウザ操作・コード修正はサブエージェントに委譲する。テスト実行（PASS/FAIL の判定を伴う）・シードデータ整備・コード修正はいずれも**判断区分**、テストソースやサーバー起動コマンドの探索は**探索区分**（`../_shared/references/subagent-policy.md`）。
