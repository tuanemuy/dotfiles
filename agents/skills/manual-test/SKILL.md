...
無くてもテストは続行する。添付せず `media/` のパスを書く。
...
証跡メディア（テストケースごとのスクリーンショットと録画）は `{output_dir}/media/` に置く。呼び出し元が PR・Issue に添付するので、ファイル名は次に固定する:
...
メインは起動前に `mkdir -p {output_dir}/media` で証跡ディレクトリを作る。サブエージェントはテストケースごとに録画1本と終了時のスクリーンショット1枚（食い違いがあればその時点でもう1枚）をそこに保存する。撮影の手順は `references/test-execution.md` の指示テンプレートにある。**メインもサブエージェントも撮ったファイルを画像として読まない。** 判断は snapshot のテキストで行い、メディアは人が後から見る証跡として PR・Issue に添付する。
...
**実行時間**: {秒}
**セッション**: verify-tc-{番号}
**証跡**: media/TC-{番号}.png / media/TC-{番号}.webm（食い違い時: media/TC-{番号}-step{N}.png）
...
**証跡**: media/（TC ごとに終了時スクリーンショットと録画）
```
...
![TC-{番号} Step {N}]({output_dir}/media/TC-{番号}-step{N}.png)
...
EOF
)" --label "bug" --attach "{output_dir}/media/TC-{番号}-step{N}.png" --attach "{output_dir}/media/TC-{番号}.webm"
```

`--attach` は Phase 1 で確認した gh の対応があるときだけ付ける。本文中の `![...]({output_dir}/media/...)` と同じパスを渡すと、gh がアップロード先の URL に書き換える。録画は本文で参照せず末尾に添付する。対応が無ければ `--attach` を外し、画像行の代わりにパスを書く。
...
- 証跡: {output_dir}/media/（スクリーンショット {数} 枚 / 録画 {数} 本）
...
agent-browser --session {s} --restore screenshot {output_dir}/media/TC-{番号}.png    # 証跡。画像として Read しない
agent-browser --session {s} --restore record start {output_dir}/media/TC-{番号}.webm  # 録画開始（セッションを開いた直後）
...
- **スクリーンショット・録画を画像として読み込まない** — 判断は実行ログと snapshot（テキスト）で行う。撮ったファイルを `Read` すると画像が以降の全ターンで context に残り続けるので、テキストで判断できないときだけ1枚読む。証跡の撮影はテストケースごとに終了時1枚・食い違い時1枚・録画1本に限り、`{output_dir}/media/` に保存して PR・Issue に添付する。それ以外のステップでは撮らない
