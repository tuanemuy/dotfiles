...
メディアはローカルに残さない。TC ごとの終了時スクリーンショットと録画は Issue・PR に添付し、この表には添付先の URL を書く。呼び出し元（issue-implement）が PR に添付する分は、添付後に呼び出し元が差し替える。

| TC | スクリーンショット | 録画 | 食い違い時 | 添付先 |
|----|-------------------|------|-----------|--------|
| TC-001 | {URL} | {URL} | - | PR #{番号} |
| TC-002 | {URL} | {URL} | {URL} | Issue #{番号} |

gh が `--attach` 非対応で添付できなかった場合は「証跡なし（gh 2.99.0 以上で添付可）」と書く。
