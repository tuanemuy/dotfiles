...
初回の実行は「録画: なし」で渡す。FAIL の再実行（SKILL.md の「FAIL の録画付き再実行」）だけ「録画: あり」で渡す。
...
3. 期待結果と食い違ったステップがあった場合:
   ...
   - 録画なしのときは同じ呼び出しでスクリーンショットを保存する: `"snapshot -i -c" "screenshot {media_dir}/TC-{番号}-step{N}.png"`。録画ありのときは撮らない（初回の分を上書きしない）
...
- **撮ったスクリーンショット・録画を画像として読み込まない** — 判断は実行ログと食い違い時の snapshot（テキスト）で行う。撮ったファイルを `Read` すると画像が以降の全ターンで context に残り続ける。テキストでは判断できないときだけ1枚読む。証跡の撮影は上の手順どおり（録画なしは終了時1枚・食い違い時1枚、録画ありは録画1本）に限り、それ以外のステップでは撮らない
...
- スクリーンショット: {media_dir}/TC-{番号}-step{N}.png（録画なしのとき）

## 証跡
- スクリーンショット: {media_dir}/TC-{番号}.png（録画なしのとき）
- 録画: {media_dir}/TC-{番号}.webm（録画ありのとき）
