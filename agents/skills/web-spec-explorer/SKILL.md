...
description: "agent-browserでWebサービスを実際に操作しながら、仕様（画面構成、ユーザーフロー、入力項目、状態遷移など）を洗い出し、spec/ディレクトリにMarkdownで構造化出力するスキル。起動条件: `/web-spec-explorer` の実行、スキル名の明示的な指定、他スキルからの委譲のみ。URL を提示された、サービスについて質問された、という状況だけでは自動起動しない。網羅性が最優先の調査は web-spec-crawler、重要箇所集中の軽量調査は本スキル。"
user-invokable: true
---
