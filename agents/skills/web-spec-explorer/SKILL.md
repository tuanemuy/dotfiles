...
description: agent-browserでWebサービスを実際に操作しながら、仕様（画面構成、ユーザーフロー、入力項目、状態遷移など）を洗い出し、spec/ディレクトリにMarkdownで構造化出力するスキル。ユーザーが「このサービスの仕様を調べて」「画面一覧を作って」「サイトの構成を把握して」「仕様を洗い出して」などと言ったとき、またはURLを提示されて仕様・要件について聞かれたとき（「どんなサービス？」のような曖昧な質問を含む）にトリガーする。網羅性が最優先の調査は web-spec-crawler、重要箇所集中の軽量調査は本スキル。
---
...
scout / explorer / flow はいずれも画面に出ているものを観察して返す作業なので**探索区分**として起動する（`../_shared/references/subagent-policy.md`）。集約と spec/ への構造化はメインが担う。
...
Phase 2〜4 の全サブエージェントに以下を渡す（各 Phase のテンプレートでは省略している）:
...
1つのサブエージェント（**探索区分**）で実行。サイト全体の構造を把握する。
...
scoutの結果をもとにサイトをセクション分けし、各セクションを別のサブエージェント（**探索区分**）に割り当てる。
