...
description: agent-browserでWebサービスを実際に操作しながら、仕様（画面構成、ユーザーフロー、入力項目、状態遷移など）を洗い出し、spec/ディレクトリにMarkdownで構造化出力するスキル。ユーザーが「このサービスの仕様を調べて」「画面一覧を作って」「サイトの構成を把握して」「仕様を洗い出して」などと言ったとき、またはURLを提示されて仕様・要件について聞かれたとき（「どんなサービス？」のような曖昧な質問を含む）にトリガーする。網羅性・完全性が最優先の仕様調査は web-spec-crawler を使う（本スキルは重要箇所集中の軽量調査）。
---
