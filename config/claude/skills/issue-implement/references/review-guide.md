...
すべての成果物は `docs/plans/{Issue番号}/` 配下に保存する。

- レビューファイル: `docs/plans/{Issue番号}/review/review-NNN.md`
- ADR: `docs/plans/{Issue番号}/adr.md`（既存ファイルに追記）
...
まず `docs/plans/{Issue番号}/plan.md` の **複雑度** メタデータを読む。
...
2. `docs/plans/{Issue番号}/plan.md` を読み、計画との整合性も確認する
3. 必要に応じて関連ファイルを読む
4. 「{レイヤー名}」の観点で厳しくレビュー
...
保存先: `docs/plans/{Issue番号}/review/review-NNN.md`（NNNは001から始まる3桁の連番）
...
4. それでもこのPRで対応できないと判断した場合のみ後回しにする:
   - `docs/plans/{Issue番号}/adr.md` に判断理由を記録する
...
レビュー中に重要な設計判断が見つかった場合、`docs/plans/{Issue番号}/adr.md` に追記する。
